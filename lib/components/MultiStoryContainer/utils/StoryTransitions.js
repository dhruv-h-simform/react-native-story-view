...
export const cubeTransition = (index, scrollX) => {
    ...
    const translateX = interpolate(scrollX.value, inputRange, [0, 0, 0], Extrapolate.CLAMP);
    const scale = interpolate(scrollX.value, inputRange, [0.79, 1, 0.78]);
    const rotateY = interpolate(scrollX.value, [offset - width, offset + width], [angle, -angle], Extrapolate.CLAMP);
    return {
